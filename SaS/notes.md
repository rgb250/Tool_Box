...
````sas
%MACRO QUERY_FCT(PERD_ARRT_INFO=, CODE_ORGN_FINN=);
	PROC SQL OUTOBS=1;
		CREATE TABLE WORK.FATCA_RCE_&CODE_ORGN_FINN._&PERD_ARRT_INFO. AS (
			SELECT
				&PERD_ARRT_INFO. AS PERD_ARRT_INFO, 
				* 
			FROM CONFO.FATCA_RCE_&CODE_ORGN_FINN._&PERD_ARRT_INFO.
		);
	QUIT;
%MEND QUERY_FCT;


%MACRO CONCAT_TABLES(USER=WORK, LISTE_CODE_ORGN_FINN=, LISTE_PERD_ARRT_INFO=, NOM_TABLE_FINALE=);
	%LOCAL i j TABLE QUERY;
	%DO j=1 %TO %SYSFUNC(COUNTW(&LISTE_PERD_ARRT_INFO, '|'));
		%LET PERD_ARRT_INFO = %SCAN(&LISTE_PERD_ARRT_INFO, &j, '|');
		%DO i=1 %TO %SYSFUNC(COUNTW(&LISTE_CODE_ORGN_FINN, '|'));
			%LET CODE_ORGN_FINN = %SCAN(&LISTE_CODE_ORGN_FINN, &i, '|');
			%QUERY_FCT(PERD_ARRT_INFO=&PERD_ARRT_INFO, CODE_ORGN_FINN=&CODE_ORGN_FINN);
			%LET TABLE = WORK.FATCA_RCE_&CODE_ORGN_FINN._&PERD_ARRT_INFO.;
			%IF &i = 1 AND &j = 1
				%THEN %DO;
			...
			%ELSE
				%DO;
					%LET QUERY = &QUERY UNION ALL SELECT * FROM &TABLE.;
				%END;
			%PUT NOTE: Appended table based on "&TABLE";	
		%END;
	%END;
	PROC SQL;
		CREATE TABLE &USER..&NOM_TABLE_FINALE. AS (
			&QUERY
		);
	QUIT;
	%DO j=1 %TO %SYSFUNC(COUNTW(&LISTE_PERD_ARRT_INFO, '|'));
