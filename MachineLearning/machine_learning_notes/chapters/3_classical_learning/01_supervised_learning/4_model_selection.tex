...
Now from Laplace's Approximation an application $f\in\mathcal{C}^{2}\left(\matbb{R}^{d}\right)$ such that $f: \matbb{R}^{d} \longrightarrow \matbb{R}$ be a smooth function. Consider as well that it has a unique maximum at $\hat{\theta}$
