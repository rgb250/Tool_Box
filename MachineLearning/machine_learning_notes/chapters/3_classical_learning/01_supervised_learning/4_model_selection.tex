...
\paragraph{Purpose}
Approximate the marginal likelihood $\prob{D} = \su{h}{}\prob{D|h}\prob{h}$
%\paragraph{Assumptions}
\paragraph{Theory}
In Bayesian model selection we want to find the model for which the posterior is higher, let's
consider $M_{k}$ the $k^{th}$ model then: 
$\prob{M_{k}|D} = \dfrac{\prob{D|M_{k}}\prob{M_{k}}}{\su{j}{}\prob{D|M_{j}}\prob{M_{j}}}$
as the key quantity is the marginal likelihood $\prob{D|M_{k}} = \Su{\Theta}{}\prob{D|\theta, M_{k}}\pi{\theta|M_{k}}d\theta$
We have then $\log\left(\prob{D|M_{k}}\right) = \Su{\Theta}{}\log\left(\prob{D|\theta, M_{k}}\right) + log\left(\pi{\theta|M_{k}}\right)d\theta$.
Now from Laplace's Approximation an application $f\in\mathcal{C}^{2}\left(\mathbb{R}^{d}\right)$ such that $f: \mathbb{R}^{d} \longrightarrow \mathbb{R}$ be a smooth function. Consider as well that it has a unique maximum at $\hat{\theta}$
