...
If we do not have strong evidence on what $\theta$ should be, it is common to \tB{use an
...
uniform distribution: $Beta(1, 1)$, but the posterior mean would then be: $\E{\theta|\mathcal{D}} =
...
In general \uB{it is advisable to perform a some kind of \textbf{sensitivity analysis}, in which one
...
The key observation is that \uB{if $p(\phi)$ is non-informative then any re-parametrization
of the prior, such as $\theta=h(\phi)$ for some function $h$ should also be non-informative}.
\begin{itemize}
	...
	\item Consider the following constraint: $p_{\phi}(\phi)\propto
        ...
        information.\\ \tB{$\mathcal{I}(\phi) \triangleq - \E{2 \times 
		\dfrac{d\log\left(p(X|\phi)\right)}{d\phi}}$}. This a measure of the
        curvature of the expected negative log likelihood and hence a \tB{measure of stability of
        the MLE}.
...
To prevent an undue influence on the result, we build \tB{priors having heavy tails}, which 
...
not flexible enough to encode our prior knowledge. However \tB{it turns out that a mixture (linear
combination) of conjugate priors is also conjugate, and seem to be a good compromise}.\\
We can \uB{represent a mixture by introducing a latent indicator variable $z$}, where $z=k$ means 
