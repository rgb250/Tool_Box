...
\mathcal{D}|\theta)p(\theta|\hat{m})d\theta$. This equation means that considering the 
hyper-parameter $\hat{m}$, that governs value taken by parameter $\theta$ we do the weighted sum
of likelihood over all possible values of $\theta$ with a weight of $p(\theta|\hat{m})$
...
$$p(\bm{\theta}|\mathcal{D},m) \propto p(\mathcal{D}|\bm{\theta},m)\times p(\bm{\theta}|m)$$
