...
\tB{$x_{t+1} \perp (x_{k})_{1\leq k\leq t-1}|x_{t}$, meaning the future is independent of the past 
...
:
\begin{center}
    \fr{$\log\left(\prob{\mathcal{D}|\bm{\theta}}\right) = \su{i=1}{n}\log\left(\prob{\bm{x_{i}}|\bm{
            \theta}}\right) = \su{j}{}N_{j}^{1}\log(\pi_{j}) + \su{j}{}\su{k}{}N_{jk}\log(A_jk)$}
\end{center}
...
\begin{center}
    \fr{$\prob{\left(\bm{z}_{t}\right)_{1\leq t\leq T}, \left(\bm{x}_{t}\right)_{1\leq t\leq T}} = 
...
\begin{itemize}
    \item \textbf{Filtering}: \tB{computing the \emph{belief state} $\prob{z_{t}|(\bm{x}_{k})_{
        1\leq k\leq t}}$ online or recursively as the data streams in}. The name "filtering" is due 
        to the noise reduction induced with the hidden state estimating  using the current 
        $\prob{z_{t}|\bm{x}_{t}}$
    \item \textbf{Smoothing}: \tB{computing $\prob{z_{t}|(\bm{x}_{k})_{1\leq k\leq t}}$ offline 
        given all the evidence} 
    \item \textbf{Fixed Lag Smoothing}: \tB{computing $\prob{z_{t-l}|(\bm{x}_{k})_{1\leq k\leq t}}$
        that is an interesting compromise between online and offline estimation}. It provides better 
        performance than filtering but incurs a slight delay. \uB{Changing the lag size, one can 
        trade off \emph{accuracy} vs \emph{delay}}
    \item \textbf{Prediction}: \tB{computing $\prob{z_{t+h}|(\bm{x}_{k}})_{1\leq k\leq t}$ where
        $h>0$ is called the \emph{prediction horizon}}. For example $h=1$ implies $\prob{z_{t+2}
    |(\bm{x}_{k})_{1\leq k\leq t}} = \su{z_{t+1}}{}\su{z_{t+1}}{}\prob{z_{t+2}|z_{t+1}}
    \prob{z_{t+1}|z_{t}} \prob{z_{t}|(\bm{x}_{k})_{1\leq k\leq t}}$
    \item \textbf{MAP estimation}: \tB{computing $\displaystyle\argmin_{(\bm{z}_{k})_{1\leq k\leq t}
            }$ which is most probable state sequence}.
    \item \textbf{Posterior samples}: \tB{if there is more that one plausible interpretation of the
            data, it can be useful to sample from the posterior $(\bm{z}_{k})_{1\leq k\leq t} 
        \hookrightarrow \prob{(\bm{z}_{k})_{1\leq k\leq t}|(\bm{z}_{k})_{1\leq k\leq t}}$}
    \item \textbf{Probability of the evidence}: \tB{compute $\prob{(\bm{x}_{k})_{1\leq k\leq t}}=
        \su{\bm{z}}{}\prob{(\bm{z}_{k})_{1\leq k\leq t}, (\bm{x}_{k})_{1\leq k\leq t}}$} meaning
        summing up over all hidden paths. Can be used to classify sequences for model-based 
        clustering, anomaly detection
\end{itemize}

\subsection{Inference HMMs}
\paragraph{The forward algorithm}
\subparagraph{Purpose} recursively \tB{computing the filtered marginals} $\prob{z_{t}|(\bm{x})_{1\leq
...
\subparagraph{Purpose}
\tB{Compute the most probable sequence of states} in a chain-structured graphical model: \tB{$z^{*}=
...
probable path}:
\begin{center}
    \fr{$\delta_{t}(j)\triangleq \displaystyle\max_{(z_{k})_{1\leq k\leq t-1}}\prob{\bm{z}_{1:T},
            z_{t}=j|\bm{x}_{1:t}}$}
\end{center}
