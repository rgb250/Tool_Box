...
$\probC{A}{B} = \dfrac{\prob{A\cap B}}{\prob{B}}$, keep in mind that it comes from the fact that
considering case where A occurs knowing that B occurred means reducing probability space $\Omega$ to
$B$. In combinatorics we could think about $\prob{A|B} =
\dfrac{\lvert A\cap B \rvert}{\lvert B \rvert} = \dfrac{\frac{\lvert A\cap B \rvert}{\lvert\Omega\rvert}}{\frac{\lvert B
\rvert} {\lvert\Omega\rvert}} = \dfrac{\prob{A\cap B}}{\prob{B}}$

\paragraph{Note on conditioning on hypotheses}
\uB{The notation $\prob{A|B}$ assumes both $A$ and $B$ are events (or outcomes of random variables)}
with well-defined probabilities in the sample space. However, \tB{in hypothesis testing, the interpretation
of conditioning differs between paradigms}:
\begin{itemize}
    \item \textbf{Frequentist framework}: A hypothesis $H_0$ is \uB{not a random variable and has no probability}
        — it is either true or false. The notation \tB{$\prob{\text{data}|H_0}$ means ``probability of the data
        computed in the probability space where $H_0$ holds''}, i.e., assuming $H_0$ is true.
        \uB{This is NOT decomposable as $\frac{\prob{\text{data} \cap H_0}}{\prob{H_0}}$} because
        $\prob{H_0}$ is undefined. \textbf{The conditioning is on an \emph{assumption}, not an event}.
    \item \textbf{Bayesian framework}: Hypotheses \uB{can be assigned probabilities via prior distributions}.
        Here, $\prob{H_0|\text{data}}$ is meaningful and can be computed using Bayes' theorem:
        $$\prob{H_0|\text{data}} = \dfrac{\prob{\text{data}|H_0} \times \prob{H_0}}{\prob{\text{data}}}$$
        In this case, $H_0$ is treated as a random variable (or event) and $\prob{H_0}$ represents
        our prior belief about the hypothesis.
\end{itemize}
\uB{In summary}: $\prob{A|B}$ with the decomposition $\prob{A|B} = \frac{\prob{A \cap B}}{\prob{B}}$
applies when both $A$ and $B$ are events with defined probabilities. When conditioning on a
\emph{hypothesis} \textbf{in frequentist statistics, the conditioning symbol $|$ represents an 
assumption rather than an event}, and Bayes' decomposition does not apply.
...
\begin{cases}
    ...
    \forall i\in \inter{1}{n},~\Prob{B_{i}} \neq 0
\end{cases} 
\Rightarrow \tR{\probC{B_{i}}{A} = \dfrac{\probC{A}{B_{i}}\times\prob{B_{i}}}{\su{k=1}{n}\probC{A}{B_{k}}\prob{B_{k}}}}
$ \\
It is used to compute the \tB{probability of a cause given its effect}.


\subsection{Moments}
They are certain quantitative measures related to the shape of the function's graph \cite{wiki:moments_mathematics}.\\
Recall that in physics to compute moment you need to \uB{store for each point the product resulting 
from the distance between mass center and that point with force applied on that point and finally 
sum them each other}.
($\mu=\su{x}{}d_{c}(x)\times F(x)$).
...
And:
\begin{center}
	$\V{\su{i=1}{n}a_{i}X_{i}} = \E{\left(\su{i=1}{n}a_{i}X_{i}\right)^{2}} - \left(\E{\su{i=1}{n}a_{i}X_{i}}\right)^{2}
	= \E{\su{i=1}{n}\su{j=1}{n}a_{i}a_{j}X_{i}X_{j}} - \left(\su{i=1}{n}a_{i}\E{X_{i}}\right)^{2}$\\
	$= \su{i=1}{n}\su{j=1}{n}a_{i}a_{j}\E{X_{i}X_{j}} - \su{i=1}{n}\su{j=1}{n}a_{i}a_{j}\E{X_{i}}\E{X_{j}}
	= \su{i=1}{n}\su{j=1}{n}a_{i}a_{j}\left(\E{X_{i}X_{j}} - \E{X_{i}}\E{X_{j}}\right)$\\
	$= \su{i=1}{n}\su{j=1}{n}a_{i}a_{j}\text{Cov}(X_{i},X_{j})
	= \su{i=1}{n}a_{i}^{2}\V{X_{i}} + 2\su{i=1}{n}\su{j>i}{n}a_{i}a_{j}\text{Cov}(X_{i},X_{j})$
\end{center}

Remember that 
\begin{center}
    $Cov(X, Y) = \E{\left[X_i - \mu_X\right]} \times \E{\left[Y_i - \mu_Y\right]} = \E{XY} -\mu_X\mu_Y$
\end{center}
...
Suppose $\prth{X}{i}{1}{n}$ is a sequence of random variables defined
on a sample space S. The sequence ``converges in probability'' to the
random variable X if, for any $\epsilon>0$
\begin{center}
	$\lm{n}{\infty}\Prob{\left|X_{n}-X\right|<\epsilon}=1$
\end{center}
\paragraph{Convergence almost surely}
Suppose the RV $X\text{ and }\prth{X}{i}{1}{n}$ is a sequence of random variables defined
on a sample space S. The sequence $X_{n}(\omega)$ ``converges almost surely'' to $X(\omega)$ if
\begin{center}
