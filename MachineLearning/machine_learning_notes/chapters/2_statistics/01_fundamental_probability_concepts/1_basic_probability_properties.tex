...
\begin{itemize}
    \item \textbf{Frequentist framework}: A hypothesis $H_0$ is \uB{not a random variable and has no probability}
        ...
        $\prob{H_0}$ is undefined. The conditioning is on an \emph{assumption}, not an event.
...
\emph{hypothesis} in frequentist statistics, the conditioning symbol $|$ represents an assumption
rather than an event, and Bayes' decomposition does not apply.
