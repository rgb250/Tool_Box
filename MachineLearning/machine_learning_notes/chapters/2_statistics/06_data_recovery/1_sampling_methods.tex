...
distribution}.
\paragraph{Theory}
Given the samples \tB{we can approximate the distribution of $f(X)$ by using the 
empirical distribution of $\left\{f(x_{s})\right\}_{1\leq s\leq S}$}\\
...
Then 2 approaches:
\begin{itemize}
    \item \emph{parametric}: \tB{generate the samples using $
            \hat{\theta}(\mathcal{D})$ as $\theta$ is unknown}
    \item \emph{non-parametric}: \tB{sample the $\left(x^{s}\right)_{1\leq s\leq S}$ 
            with replacement from the original $\mathcal{D}$ and then compute induced
            distribution
}\end{itemize}
