...
\paragraph{Frequentist}
\tB{$\chi^{2} = \su{i=1}{k}\dfrac{(O_{i} - E_{i})^{2}}{E_{i}}$}
where $E_i = Np_i$ is the expected count and:
$\begin{cases}
    O_{i}\text{: \uB{observed count} of type }i \\
    E_{i}\text{: \uB{expected count} of type }i \\
    N\text{: total number of observations}\\
    k\text{: number of categories}\\
    p_{i}\text{: \uB{expected proportion} of type }i\text{ under } H_0
\end{cases}$
...
$$X_{ij} = \mathbbm{1}_{\{\text{trial } j \text{ results in category } i\}} =
\begin{cases} 1 & \text{if trial } j \to \text{category } i \\ 0 & \text{otherwise} \end{cases}$$

\begin{center}
\begin{tabular}{|c|*{6}{c}|c|}
    \hline
    & \textbf{Trial 1} & \textbf{Trial 2} & $\cdots$ & \textbf{Trial $j$} & $\cdots$ & \textbf{Trial $N$} & \textbf{Row sum} \\
    \hline
    \textbf{Category 1} & $X_{11}$ & $X_{12}$ & $\cdots$ & $X_{1j}$ & $\cdots$ & $X_{1N}$ & $O_1 = \su{j=1}{N}X_{1j}$ \\
    \textbf{Category 2} & $X_{21}$ & $X_{22}$ & $\cdots$ & $X_{2j}$ & $\cdots$ & $X_{2N}$ & $O_2 = \su{j=1}{N}X_{2j}$ \\
    $\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ \\
    \textbf{Category $i$} & $X_{i1}$ & $X_{i2}$ & $\cdots$ & $X_{ij}$ & $\cdots$ & $X_{iN}$ & $O_i = \su{j=1}{N}X_{ij}$ \\
    $\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ \\
    \textbf{Category $k$} & $X_{k1}$ & $X_{k2}$ & $\cdots$ & $X_{kj}$ & $\cdots$ & $X_{kN}$ & $O_k = \su{j=1}{N}X_{kj}$ \\
    \hline
    \textbf{Col sum} & $1$ & $1$ & $\cdots$ & $1$ & $\cdots$ & $1$ & $N$ \\
    \hline
\end{tabular}
\end{center}
...
\begin{itemize}
    \item Each column sums to 1: $\su{i=1}{k}X_{ij} = 1$ (each trial falls in exactly one category)
    \item \textbf{Observed count}: \tR{$O_i = \su{j=1}{N}X_{ij}$ (sum of row $i$)}
    \item \tB{$X_{ij} \sim \text{Bernoulli}(p_i)$ with $\mathbb{E}[X_{ij}] = p_i$}
    \item \textbf{Expected count}: $E_i = \mathbb{E}[O_i] = \mathbb{E}\left[\su{j=1}{N}X_{ij}\right]
        = \su{j=1}{N}\mathbb{E}[X_{ij}] = \su{j=1}{N}p_i = Np_i$
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Multinomial distribution}: Under $H_0$, observed counts $(O_1, \ldots, O_k)$ follow
        a multinomial distribution with:
        \begin{itemize}
            \item $\mathbb{E}[O_i] = Np_i = E_i$
            \item $\text{Var}(O_i) = Np_i(1-p_i) = E_i(1-p_i)$
            \item $\text{Cov}(O_i, O_j) = -Np_ip_j$ for $i \neq j$ (negative correlation)
        \end{itemize}
    \item \textbf{Why $\sqrt{E_i}$ and not $\sqrt{E_i(1-p_i)}$?}\\
        The true standard deviation is $\sigma_i = \sqrt{E_i(1-p_i)} \neq \sqrt{E_i}$.
        The $\chi^2$ statistic is \emph{not} a simple sum of squared standardized normals,
        but a specific \textbf{quadratic form} that accounts for the covariance structure.
        Let $Z_i = \dfrac{O_i - E_i}{\sqrt{E_i}}$, then $\chi^2 = \su{i=1}{k}Z_i^2 = \mathbf{Z}^T\mathbf{Z}$ with:
        $$\text{Var}(Z_i) = \dfrac{E_i(1-p_i)}{E_i} = 1-p_i \quad\text{and}\quad \text{Cov}(Z_i, Z_j) = \dfrac{-Np_ip_j}{N\sqrt{p_ip_j}} = -\sqrt{p_ip_j}$$
        The covariance matrix is $\Sigma = I - \sqrt{\mathbf{p}}\sqrt{\mathbf{p}}^T$ where $\sqrt{\mathbf{p}} = (\sqrt{p_1}, \ldots, \sqrt{p_k})^T$.\\
        $\Sigma$ is \textbf{idempotent} ($\Sigma^2 = \Sigma$) because $\sqrt{\mathbf{p}}^T\sqrt{\mathbf{p}} = \su{i=1}{k}p_i = 1$:
        $$\Sigma^2 = I - 2\sqrt{\mathbf{p}}\sqrt{\mathbf{p}}^T + \sqrt{\mathbf{p}}\underbrace{(\sqrt{\mathbf{p}}^T\sqrt{\mathbf{p}})}_{=1}\sqrt{\mathbf{p}}^T = I - \sqrt{\mathbf{p}}\sqrt{\mathbf{p}}^T = \Sigma$$
        For idempotent $\Sigma$: $\mathbf{Z}^T\mathbf{Z} \xrightarrow{d} \chi^2_{\text{rank}(\Sigma)}$ with $\text{rank}(\Sigma) = \text{tr}(\Sigma) = k - 1$.
    \item \textbf{Chi-squared distribution}: Recall that if $Z_1, \ldots, Z_m \stackrel{iid}{\sim} \mathcal{N}(0,1)$, then
        $\su{i=1}{m}Z_i^2 \sim \chi^2_m$
    \item \textbf{Degrees of freedom}: $k-1$ (not $k$) due to the constraint $\su{i=1}{k}O_i = N$,
        which introduces a linear dependency among the $k$ counts.
\end{enumerate}
This explains \uB{why expected counts must be sufficiently large} ($E_i \geq 5$): the test relies
