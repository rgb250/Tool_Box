...
\begin{itemize}
    ...
    \item \textbf{Observed count}: $O_i = \su{j=1}{N}X_{ij}$ (sum of row $i$)
    \item $X_{ij} \sim \text{Bernoulli}(p_i)$ with $\mathbb{E}[X_{ij}] = p_i$
...
\begin{enumerate}
    ...
    \item \textbf{Why $\sqrt{E_i}$ and not $\sqrt{E_i(1-p_i)}$?}\\
        ...
        $$\chi^2 = \su{i=1}{k}\dfrac{(O_i - E_i)^2}{E_i} \xrightarrow{d} \chi^2_{k-1}$$
        The factors $(1-p_i)$ and covariances ``cancel out'' in this quadratic form construction.
