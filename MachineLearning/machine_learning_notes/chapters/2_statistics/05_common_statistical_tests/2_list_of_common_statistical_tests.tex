...
\paragraph{Frequentist}
\tB{$\chi^{2} = \su{j=1}{k}\dfrac{(O_{j} - E_{j})^{2}}{E_{j}}$}
...
$$X_{ij} = \mathbbm{1}_{\{\text{trial } i \text{ results in category } j\}} =
...
\begin{enumerate}
    \item \textbf{Multinomial distribution}: Under $H_0$, observed counts \tB{$(O_1, \ldots, O_k)$ follow a multinomial distribution} with:
        \begin{itemize}
    ...
    \item \textbf{Why $\sqrt{E_j}$ and not $\sqrt{E_j(1-p_j)}$?}\\
        ...
        \textit{Step 1 -- Define $Z_j$}: Let \tB{$Z_j = \dfrac{O_j - E_j}{\sqrt{E_j}}$}, so
        $\chi^2 = \su{j=1}{k}Z_j^2 = \bm{Z}^T\bm{Z}$ where $\bm{Z} = (Z_1, \ldots, Z_k)^T \in \mathbb{R}^k$ is a column vector.

        \textit{Step 2 -- The $Z_j$ are not standard normals}: Since we divided by $\sqrt{E_j}$
        ...
        \textit{Step 3 -- Covariance matrix}: Let $\sqrt{\bm{p}} = (\sqrt{p_1}, \ldots, \sqrt{p_k})^T \in \mathbb{R}^k$ (column vector).
        ...
        \textit{Step 4 -- $\bm{\Sigma}$ is idempotent}: Since $\sqrt{\bm{p}}^T\sqrt{\bm{p}} = \su{j=1}{k}p_j = 1$ (a scalar):
        ...
        \textit{Step 5 -- Cochran's theorem}: \tB{For $\bm{Z} \sim \mathcal{N}(\bm{0}, \bm{\Sigma})$ with $\bm{\Sigma}$
    ...
    \item \textbf{Chi-squared distribution}: Recall that if $Z_1, \ldots, Z_m \stackrel{iid}{\sim} \mathcal{N}(0,1)$, then
...
\end{enumerate}
This explains \uB{why expected counts must be sufficiently large} ($E_j \geq 5$): the test relies
on the normal approximation to the multinomial, which requires large samples.
...
\begin{itemize}
    \item in test of independence, for a small sample size use rather Fisher's exact
        test.
\end{itemize}
...
\paragraph{Assumptions}
\begin{itemize}
    \item Distribution $F$ is symmetric around its median
    \item Observations are independent
    \item Data is at least ordinal
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Non-parametric: no normality assumption required
    \item More powerful than the sign test
    \item Robust to outliers
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Requires symmetry assumption
    \item Less powerful than $t$-test when normality holds
\end{itemize}
...
\paragraph{Assumptions}
\begin{itemize}
    \item All observations from both groups are independent
    \item Values are at least ordinal
    \item $H_{0}$: the distributions of both populations are identical
    \item $H_{1}$: the distributions of the 2 populations are different
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Non-parametric: no normality assumption required
    \item Can be used with ordinal data
    \item Related to AUC (Area Under ROC Curve)
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Less powerful than $t$-test when normality holds
    \item Only tests for stochastic dominance, not specific location shift
\end{itemize}
...
\begin{itemize}
    \item Independence
    \item All groups should have the same shape of distributions
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Non-parametric: no normality assumption required
    \item Extension of Mann-Whitney to $k > 2$ groups
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Less powerful than ANOVA when normality holds
    \item Only detects if groups differ, not which groups differ (requires post-hoc tests)
\end{itemize}
...
\paragraph{Assumptions}
\begin{itemize}
    \item Blocks are mutually independent
    \item Observations can be ranked within blocks
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Non-parametric alternative to repeated-measures ANOVA
    \item No normality or sphericity assumptions
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Less powerful than repeated-measures ANOVA when assumptions hold
    \item Requires post-hoc tests to identify which treatments differ
\end{itemize}
...
where $\rho$ is the Pearson correlation coefficient applied to the rank variables.

\paragraph{Assumptions}
\begin{itemize}
    \item Observations are independent
    \item Variables are at least ordinal
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Robust to outliers (uses ranks)
    \item Detects monotonic relationships (not just linear)
    \item No normality assumption required
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Less powerful than Pearson's $r$ for linear relationships with normal data
    \item Sensitive to ties in the data
\end{itemize}
...
Sample estimator: $r = \dfrac{\su{i=1}{n}(x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\su{i=1}{n}(x_i - \bar{x})^2 \su{i=1}{n}(y_i - \bar{y})^2}}$

\paragraph{Assumptions}
\begin{itemize}
    \item Both variables are continuous
    \item Linear relationship between variables
    \item Bivariate normality (for inference)
...
\begin{itemize}
    ...
    \item Optimal for detecting linear relationships in normal data
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Not robust to outliers
    \item Only detects linear relationships
    \item Sensitive to non-normality
\end{itemize}
...
\paragraph{Strengths}
\begin{itemize}
    \item Can compare more than 2 groups simultaneously
    \item Controls family-wise Type I error rate
    \item Well-developed post-hoc procedures available
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Sensitive to violations of normality (especially with small samples)
    \item Sensitive to unequal variances (use Welch's ANOVA instead)
    \item Only detects that groups differ, not which ones (requires post-hoc tests)
\end{itemize}
...
\begin{itemize}
    ...
    \item Normality of the population (or large sample size by CLT)
    \item For two-sample: equal variances (or use Welch's $t$-test)
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item Most powerful test for comparing means under normality
    \item Well-understood and widely available
    \item Robust to mild departures from normality with large samples
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Sensitive to outliers
    \item Requires normality assumption for small samples
    \item Two-sample version sensitive to unequal variances
\end{itemize}
...
where $X_D = X_1 - X_2$ is the difference between paired observations.

\paragraph{Assumptions}
\begin{itemize}
    \item Pairs are independent
    \item Differences are normally distributed
\end{itemize}

\paragraph{Strengths}
\begin{itemize}
    \item More powerful than independent $t$-test when data is paired
    \item Controls for individual differences
\end{itemize}

\paragraph{Weaknesses}
\begin{itemize}
    \item Requires paired/matched data
    \item Sensitive to outliers in the differences
\end{itemize}
